...
function ExtractionLayout(props) {
    ...
    const { appflyteData, setHomePagePreview, extractionFiles, extractionTasks, documentTypeData, settingsJsonStatus,
        ...
        fetchDomainDictionarySchema, onCancel, onSave, fetchExtractionTasks
    } = props;
        ...
        try {
            ...
            if (upload_response.status === 200) {
                ...
                if (save_response.status === 200) {
                    const all_files = await fetchAllExtractionFiles(appflyte_details, documentTypeData.__auto_id__);
            ...
            if (validateSchema) {
                ...
                if (validation_response?.is_valid) {
                    ...
                    if (templateSettingsRes.status === 200) {
                        ...
                        onSave()
                        await fetchDomainDictionarySchema(appflyteData, documentTypeData.__auto_id__)
                    } else {
    ...
    const handleExtractionFileSelection = async (item, allFiles, type) => {
        setFileLoading(true);
        try {
            if (!window.pdfjsLib) {
            ...
            const selectedFile = allFiles.find(file => file?.payload?.__auto_id__ === item);
            if (!selectedFile) {
                return;
            }
            ...
            const runExtraction = async () => {
                ...
                fetchExtractionTasks(appflyte_details, documentTypeData.__auto_id__);
            };
            ...
            //         if (!extractionResponse) await runExtraction();
            //     } else {
            //         await runExtraction();
            //     }
            // } else {
    ...
    useEffect(() => {
        if (selectedExtractionFile) {
            ...
            handleExtractionFileSelection(selectedExtractionFile, extractionFiles, 'init');
        }
    ...
    const handleAddNewFile = async (event) => {
        try {
            ...
            const check_file = extractionFiles?.find(item => item?.payload?.file_name === file?.name);
            if (check_file) {
                alert("A file with the same name already exists. Please choose a different name or try again.");
                return
            }
            ...
            if (file_selected) {
                setRevalidateStatus(false);
                const all_files = await handleFileUpload(file_selected);
                const filteredFiles = all_files.find(file => file?.payload?.file_name === file_selected[0]?.name);
                setSelectedExtractionFile(filteredFiles?.payload?.__auto_id__)
                return
            }
        } catch (error) {
                        ...
                        < Box display="flex" alignItems="center">
                            ...
                            {revalidateStatus &&
                                (<Button
                                    ...
                                    onClick={() => handleExtractionFileSelection(selectedExtractionFile, extractionFiles, 're-test')}
                                ...
                                <Box display={'flex'} alignItems={'center'}>
                                    <FormControl>
                                        <Autocomplete
                                            ...
                                            options={extractionFiles || []}
                                            getOptionLabel={(option) => option?.payload?.file_name || ""}
                                            value={extractionFiles?.find(file => file?.payload?.__auto_id__ === selectedExtractionFile) || null}
                                            onChange={(event, newValue) => {
                                                setSelectedExtractionFile(newValue?.payload?.__auto_id__);
                                                setRevalidateStatus(false);
                                            }}
                                            disabled={fileLoading || isOcrProcessing || extractionLoading || loading}
                                            sx={{
                                            ...
                                            isOptionEqualToValue={(option, value) =>
                                                option?.payload?.__auto_id__ === value?.payload?.__auto_id__
                                            }
                                            renderOption={(props, option) => (
                                                <li {...props} key={option?.payload?.__auto_id__}>
                                                    <Typography sx={styles.paraText}>
                                                        {option?.payload?.file_name}
                                                    </Typography>
                                                </li>
                                            )}
                                            renderInput={(params) => (
